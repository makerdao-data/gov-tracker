...
def proxy_data_view(sf, proxy):
    ...
    try:
        ...
        return dict(
            status="success",
            data=dict(operations=operations_data, operations_num=operations_num),
        )
    ...
    try:
        ...
        if not (len(proxy) == 42 and proxy[:2] == "0x"):
            return render_template(
                "unknown.html", object_name="proxy", object_value=proxy
            )

        proxy_data = sf.execute(
            f"""
        ...
        return render_template(
            ...
            last_vote=proxy_data[9].strftime("%Y-%m-%d %H:%M:%S")
            if proxy_data[9]
            else "",
