...
def poll_data_view(sf, poll):
    ...
    try:
        ...
        for i in operations:
            ...
            operations_supplement = sf.execute(
                operations_supplementary_query
            ).fetchall()
        ...
        if options:
            options = {
                key: dict(title=value, votes=0, stake=0)
        ...
        for operation in operations:
            ...
            options_list = "<br>".join(
                [
                    str(options[vote]["title"]) if vote in options else "Not valid"
                    for vote in votes
                ]
            )
        ...
        options_list = [
            ...
            for key, o in options_list
        ]
        options_list = [
            ["Option", "Option name", "Stake (MKR)", "Votes"]
        ] + options_list

        options_table = html_table(
            options_list,
            table_id="options",
            widths=["60px", None, "90px", "90px"],
            expose=[0],
            tooltip=False,
        )
    ...
    try:
        ...
        if not poll.isnumeric():
            return render_template(
                "unknown.html", object_name="poll", object_value=poll
            )
        ...
        return render_template(
            "poll.html", yay=poll, title=title, refresh=last_update[0].__str__()[:19]
        )
