...
def main_page_data(sf):
    ...
    try:
        ...
        executives_list = [
            ["Spell", "Since", "Description", "Approval<br>(MKR)", "Voters<br>(#)"]
        ] + executives_list
        ...
        for poll in polls_records:
            ...
            if poll[0] not in polls:
                polls[poll[0]] = dict(
                    votes=0, stake=0, max_stake=0, winning=None, since=poll[2]
                )
        ...
        polls_list = [
            ...
            for p in polls
            if p[0] in polls_metadata
        ]

        polls_no_votes = sf.execute(
            f"""
...
def main_page_view(sf):
    ...
    try:
        ...
        return render_template("main.html", refresh=last_update[0].__str__()[:19])
